...
return length(a) == 0 ? zero(base_ring(a)) : coeff(a, length(a) - 1) end
...
return z end
function mul_classical(a::PolyElem{T}
, b::PolyElem{T}
) where T <: RingElement
lena = length(a)
lenb = length(b)
...
check_parent(a, b)
   return mul_classical(a, b) end
function *(a::Union{Integer, Rational, AbstractFloat}
, b::PolynomialElem)
len = length(b)
z = parent(b)()
fit!(z, len)
...
return r end
function divexact(f::PolyElem{T}
, g::PolyElem{T};
 check::Bool=true) where T <: RingElement
check_parent(f, g)
...
return divexact(b, d) end
function content(a::PolyElem)
z = base_ring(a)() 
# normalise first coefficient
   for i = 1:length(a)
   z = gcd(z, coeff(a, i - 1))
end
return z end
function PolynomialRing(R::Ring, s::Symbol;  cached::Bool = true)
return Generic.PolynomialRing(R, s; cached=cached) end 
